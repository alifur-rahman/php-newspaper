...
(function (b) {
    ...
    b(function () {
        ...
        b("[data-marquee]").marquee({ direction: g ? "right" : "left", delayBeforeStart: 0, duration: g ? 8e3 : 2e4, pauseOnHover: !0, startVisible: !0 });
        ...
        b("[data-countdown]").each(function () {
            ...
            a.countdown(a.data("countdown"), function (a) {
                b(this).html(
                    "<ul>" + a.strftime("<li><strong>%D</strong><span>DAYS</span></li><li><strong>%H</strong><span>HOURS</span></li><li><strong>%M</strong><span>MINUTES</span></li><li><strong>%S</strong><span>SECONDS</span></li>") + "</ul>"
                );
            });
        });
        var e = function (a, b) {
        ...
        h.on("click", function (a) {
            setTimeout(function () {
                l.on("click.headerNavbarToggle", function (a) {
                    a.$target = b(a.target); 
                    0 === a.$target.parents("#headerNav2").length && (nextmenu.removeClass("in"), h.addClass("collapsed"), l.off("click.headerNavbarToggle"));
                });
            }, 200);
        });  
        ...
        b.fn.initMap = function () {
            if ("undefined" !== typeof google) {
                var a = new google.maps.Map(this[0], { center: { lat: this.data("map-latitude"), lng: this.data("map-longitude") }, zoom: this.data("map-zoom"), scrollwheel: !1, disableDefaultUI: !0, zoomControl: !0 });
                if ("undefined" !== typeof this.data("map-marker")) {
                    var b = this.data("map-marker"),
                        c = 0;
                    for (c; c < b.length; c++) new google.maps.Marker({ position: { lat: b[c][0], lng: b[c][1] }, map: a, animation: google.maps.Animation.DROP, draggable: !0 });
                }
            }
        };
        c.each(function () {
...
$(document).ready(function(){
    $('#sec_menu_close').click(function(){
        $('#headerNav2').removeClass('in');
        $('#headerNav2').addClass('collapsed');
...
function onSearch(){
    ...
    if(search != ""){
        window.location.href = "/search/"+search.replace(/ /g, "-");
    }
}


//  repeated ads 
var adsArea = document.querySelectorAll('.Al_ads_area');
for (let x = 0; x < adsArea.length; x++) {
    var getAdsItem = adsArea[x].querySelectorAll('.al-ads-item');
    adsLoop(adsArea[x],getAdsItem); 
}

function adsLoop(adsArea,adsItem){
    var timeout = 5000;
    adsItem[0].classList.add('Ad_active');
    for (let index = 0; index < adsItem.length; index++) {
        setTimeout(() => {
            adsArea.querySelector('.Ad_active').classList.remove("Ad_active");
            adsItem[index].classList.add('Ad_active');

            setTimeout(() => {
                if(adsItem.length - 1 == index){
                    adsArea.querySelector('.Ad_active').classList.remove("Ad_active");
                    adsLoop(adsArea,adsItem);
                    console.log(adsItem.length - 1);
                }
            }, timeout);
        },index * timeout);
    }
...
function socialShare(){
    var sFacebook = document.getElementById('sFacebook');
    var sTwitter = document.getElementById('sTwitter');
    var sLinkedin = document.getElementById('sLinkedin');
    var url =  window.location.href;
    sFacebook.setAttribute('href', 'https://www.facebook.com/sharer/sharer.php?u=https%3A//'+url+'/');
    sTwitter.setAttribute('href', 'https://twitter.com/intent/tweet?text=https%3A//'+url+'/');
    sLinkedin.setAttribute('href','https://www.linkedin.com/shareArticle?mini=true&url=&title=&summary=&source=https%3A//'+url+'/');
}
socialShare();
